...
var x{s in S, r in R, I, A[s,r]} binary;       #//発表するかどうか(個人単位)
...
minimize Objective:
 sum {s in S} p1[s] + sum {d in D, l in L} p2[d,l] + sum {d1 in D, d2 in D, l in L : d1 != d2 } p3[d1,d2,l] + sum{(i1, i2) in C} p4[i1,i2];


#//全学生が1回発表する
subject to AllStudentOnePublish {i in I}:
 sum {s in S, r in R, a in A[s, r]} x[s,r,i,a] == 1;

#//学生は教員・自身が共に参加可能なセッションで発表できる
subject to AvailableSession {s in S, r in R, l in L, i in I, a in A[s, r] : b[l,i] == 1}:
...
subject to NumOfSession {s in S, r in R}:
 n_s[s,r] == sum {i in I, a in A[s,r]} x[s,r,i,a];
...
subject to NumOfPublishers {d in D, l in L}:
 n_d[d,l] == sum {s_ in S_[d], r in R, i in I, a in A[s_,r] :b[l,i] == 1} x[s_,r,i,a];
...
subject to DifferenceOrder {s in S, r in R, a in A[s,r]}:
...
subject to IdCombination_L {s in S, r in R, a in A[s,r], (i1, i2) in C}:
 x[s,r,i1,a] - x[s,r,i2,a] <= p4[i1,i2];

subject to IdCombination_R {s in S, r in R, a in A[s,r], (i1, i2) in C}:
 -p4[i1,i2] <= x[s,r,i1,a] - x[s,r,i2,a];
