...
minimize Objective:
 sum {s in S} p1[s] + sum {d in D, l in L} p2[d,l] + sum {d1 in D, d2 in D, l in L : d1 != d2 } p3[d1,d2,l] + sum{(i1, i2) in C} p4[i1,i2] + sum{(l_1,l_2) in G} p5[l_1,l_2];
...
subject to IdCombination_L {s in S, r in R, a in A[s], (i1, i2) in C}:
 x[s,r,i1,a] - x[s,r,i2,a] <= p4[i1,i2];

subject to IdCombination_R {s in S, r in R, a in A[s], (i1, i2) in C}:
 -p4[i1,i2] <= x[s,r,i1,a] - x[s,r,i2,a];

#//発表を聞きたい研究室の学生の発表スロットができるだけ重ならない
subject to MakeGallery_x {s in S, a in A[s], i_1 in I, i_2 in I, (l_1, l_2) in G : b[l_1,i_1] == 1 && b[l_2,i_2] == 1}:
